...
- [x] Billing enabled and Grounding with Google Search verified working 2026-07-10, grounded answer came back with real cited sources. The paid tier includes 5,000 grounded prompts per month free, so grounding costs zero at our scale. Token costs are the only real spend, roughly 2 to 3 cents of input per minute of video analyzed, ten dollars covers the day. Every pipeline pillar (video, claims, grounding, function calling, market data, ingestion) is now proven on the real keys. Keys live in the local `.env`, gitignored, this repo is public, never commit them.
