...
- [ ] BLOCKER UNTIL BILLING: Grounding with Google Search returns 429 on the free tier, it needs a card on the key. Plain calls, video, and function calling all work free, but stage 3's search verification will not run until billing is enabled. This is the one must-do before hack day.
